...
def gps_record(filename,contours,latitude,longitude):#latitude,longitude is the image's gps info
    ...
    for i in range (0,N):
        min_=cv.minAreaRect(contours[i]) #box  center is （min_[0][0],min_[0][1]）
        G_latitude=latitude+(min_[0][0]-center[0])*(-ratio) #center(of image) is a global parameter
        G_longitude=longitude+(min_[0][1]-center[1])*(-ratio)
        try:
...
#return dic
def dict_return(filename,contours):
    N= np.array(contours).shape[0]#box number
    dict_={}
    dict_in={}
    for i in range (0,N):     
        dict_point={}
        min_=cv.minAreaRect(contours[i])
        center=[min_[0][0],min_[0][1]]#center
        box = cv.boxPoints(min_) 
        box = np.int0(box)#box：[ [x0,y0], [x1,y1], [x2,y2], [x3,y3] ]
        dict_point['Center']=center
        dict_point['Rect']=box
        dict_in['Box%s'%i]=dict_point
        
    dict_[filename]=dict_in

    return dict_


def pixel_record(filename,contours):       
    N= np.array(contours).shape[0]
    for i in range (0,N):
        min_=cv.minAreaRect(contours[i])
        center=[min_[0][0],min_[0][1]]#center
        ...
        box = np.int0(box)#box：[ [x0,y0], [x1,y1], [x2,y2], [x3,y3] ]
        try:
            with open (box_pixel_point,'a') as f:
                f.write('Filename:%s'%filename+'\t'+'Box:%s'%N+'\t'+'No.%d'%i+'\t'+'center:({0[0]},{0[0]})'.format(center)+'\t'+'rect_point:({0[0]},{0[1]},{0[2]},{0[3]})\n'.format(box))
        except IOError:
            logging.info('IO Error when write center_pixel info into txt file!')
            pass
##################################  
...
def main(arg):
    try:
        TM=ini()
        logging.info('path=%s'%arg)#arg=filename.jpg
        
        TM0 = timeit.default_timer()
        global center,ratio,path_image,path_result,box_pixel_point
        center=(1500,2000)
        ratio=0.36e-6 #pixel to GPS
        
        #path_image='./images_feed/'
        path_result='./results/'
        #gps_record_path='./GPS_record/'
        #box_gps=gps_record_path+TM+'box_gps.txt'
        record_path='./points_record/'
        box_pixel_point=record_path+TM+'_box_point.txt'
        
        #filename=os.path.splitext(files)[0];#文件名
        filePath=arg#path_image+files#单个文件路径
        filename=os.path.basename(filePath)#提取文件名 
        # or 
        #filePath.split("/")[-1]
        
        
        logging.info('%%%%%%%%%%%%%%%%%%%%%%%%processing image:%s'%(filename))
        ...
        pixel_record(filename,contours_update)
        dict_out=dict_return(filename,contours)
        ##################################
        ##################################
        
        '''try:
                                    cv.imwrite(path_result+'%s'%TM+filename,image_with_text)
                                except Exception :
        ...
        #logging.info('%s is processed, delete it from images_input.'%filename)
        #os.remove(filePath)
